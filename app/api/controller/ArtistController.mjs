...
router.get('/top', async (req, res) => {
  try {
    ...
    const artists = await ArtistsFetchDAO.findAll(access_token);
    res.json(artists);
  } catch (err) {
...
router.get('/top/popularity', async (req, res, next) => {
  try {
    ...
    const artists = await ArtistsFetchDAO.findAll(accessToken);
...
router.get('/:artistId', async (req, res) => {
  try {
    ...
    const artist = await ArtistsFetchDAO.findArtist([artistId], access_token);
    res.json(artist);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/:artistId/albums', async (req, res) => {
  try {
    ...
    const albums = await ArtistsFetchDAO.findArtistAlbums(artistId, limit, access_token);
    res.json(albums);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/:artistId/top', async (req, res) => {
  try {
    const access_token = req.session.access_token;
    const artistId = req.params.artistId;
    const topTracks = await ArtistsFetchDAO.findTopArtist(artistId, access_token);
    res.json(topTracks);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
